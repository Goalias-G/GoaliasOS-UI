...
import type { FormField, TableColumn, ChatModel } from '@/types'

// ==================== 状态 ====================
const loading = ref(false)
const dataList = ref<ChatModel[]>([])
...
async function handleBatchDelete() {
  ...
  if (!confirm(`确定要批量删除选中的 ${count} 个模型吗？此操作不可恢复。`)) {
    return
  }
...
const columns: TableColumn[] = [
  ...
  { key: 'category', label: '模型标识', width: '120px' },
  { key: 'providerName', label: '提供商', width: '120px' },
  {
    key: 'modelShow',
    label: '状态',
    width: '80px',
    formatter: (value: string) => (value === '0' ? '启用' : '禁用'),
  },
  {
    key: 'enableSearch',
    label: '支持联网搜索',
...
const formFields: FormField[] = [
  ...
  },
  {
    ...
    label: '模型价格',
    type: 'number',
    placeholder: '请输入模型价格（每百万输出Token）',
...
async function loadData() {
  try {
    ...
    const response = await chatModelApi.list(
      searchKeyword.value ? { modelName: searchKeyword.value } : {},
...
function handleAdd() {
  ...
  formData.value = {
    modelShow: '0', // 默认启用状态
    enableSearch: '0', // 默认禁用搜索
    category: 'chat', // 默认对话模型
  }
  modalVisible.value = true
}

function handleEdit(row: ChatModel) {
  modalMode.value = 'edit'
  formData.value = { ...row }
  modalVisible.value = true
}

async function handleDelete(row: ChatModel) {
  if (!confirm(`确定要删除模型 "${row.modelName}" 吗？`)) {
    return
  }
...
<template>
  <div class="chat-model-module">
    ...
    <FormModal
      ...
      :title="modalMode === 'add' ? '新增聊天模型' : '编辑聊天模型'"
      :mode="modalMode"
      :form-data="formData"
      :fields="formFields"
